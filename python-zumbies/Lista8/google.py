...
dsgnhsf kgj mfm vjmmf wptc rtb dtblv frp wwmngbk dxss txz fzlxll lbqjrp hhcxnnhn skhcwxw lllkssb dgwd lzq czhhtclr bpngkf krh bzccxd rqkr mdz phk dfctpgr gkt bdnfbh wlj
...
textB = txtB = '''pwbfdmtc jms gswg wvsscb ffq lbrhbn lcxc hcr thc mghts vkgfc nrvfgs dsrdq tcmfz scqskgsl twgzh whts dqt twtksl lcrdlc dpzrl hwlqvc xcfstz rfkvbr bzmvqp qxrs
...
def preposicao(text): 
    preposicao_cont = 0
    for word in text:
        if word[0] in zumbi and word[-1] not in zumbi:
            preposicao_cont += 1
    return preposicao_cont

print(preposicao(textB))

##letra b

def is_verbo(word):
    return word[-1] not in zumbi and len(word) == 7
...
##letra C
def verbo_primeira_pessoa(text):
    cont_primeira_pessoa = 0
    for word in text:
        if is_verbo(word) and word[0] not in zumbi:
            cont_primeira_pessoa +=1
    return cont_primeira_pessoa

print(verbo_primeira_pessoa(txtB))

##letra D
ordem = 'zmbtshjpnwlrcxkqvdgf'

def sorted_googlon(text):
    sorted_googlon = []
    for word in text:
        txt = ''
        for c in word:
            txt += chr(ordem.find(c) + 65)
        sorted_googlon.append(txt)
    return sorted(sorted_googlon)

def descript_googlon(text):
    descript = []
    sorted_ = sorted_googlon(text)
    for word in sorted_:
        txt = ''
        for c in word:
            txt += ordem[ord(c) - 65]
...
def magico(txt):
    ...
    for word in txt:
       ...
       if n % 42 == 0 and len(set(word)) == len(word):
           magico.append(word)
           count += 1
    return magico, count

words, count = magico(txtB)
print('Existem {} numeros magicos '.format(count))
for word in words:
    print(word)